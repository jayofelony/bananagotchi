...
pwnagotchi: $(SDIST) builder/bananagotchi.json.pkr.hcl builder/bananagotchi.yml $(shell find builder/data -type f)
	...
	cd builder && sudo /usr/bin/packer init && sudo $(UNSHARE) /usr/bin/packer build -var "pwn_hostname=$(PWN_HOSTNAME)" -var "pwn_version=$(PWN_VERSION)" bananagotchi.json.pkr.hcl
