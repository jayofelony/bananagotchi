...
def setup_mounts(config):
    ...
    for name, options in fs_cfg['mounts'].items():
        ...
        target = os.path.join('/run/bananagotchi/disk/', os.path.basename(options['mount']))
        ...
        if interval:
            logging.debug("[FS] Starting thread to sync %s (interval: %d)", options['mount'], interval)
            _thread.start_new_thread(m.daemonize, (interval,))
        else:
            logging.debug("[FS] Not syncing %s, because interval is 0", options['mount'])
...
class MemoryFS:
    ...
    def mount(self):
        ...
        if self.zram and self.zdev is not None:
            if os.system(f"mount -t {self.zram_fs_type} -o nosuid,noexec,nodev,user=bananagotchi /dev/zram{self.zdev} {self.mountpoint}/"):
                return False
        else:
            if os.system(f"mount -t tmpfs -o nosuid,noexec,nodev,mode=0755,size={self.size} bananagotchi {self.mountpoint}/"):
                return False
