...
import bananagotchi
import bananagotchi.grid as grid
import bananagotchi.ui.web as web
...
class Handler:
    ...
    def plugins(self, name, subpath):
        ...
        if name == 'upgrade' and request.method == 'POST':
            ...
            os.system(f"bananagotchi plugins update && bananagotchi plugins upgrade {request.form['plugin']}")
    ...
    def shutdown(self):
        try:
            return render_template('status.html', title=bananagotchi.name(), go_back_after=60,
                                   message='Shutting down ...')
        finally:
            _thread.start_new_thread(bananagotchi.shutdown, ())
    ...
    def reboot(self):
          try:
              ...
              _thread.start_new_thread(bananagotchi.reboot, ())
    ...
    def restart(self):
        ...
        try:
            return render_template('status.html', title=bananagotchi.name(), go_back_after=30,
                                   message='Restarting in %s mode ...' % mode)
        finally:
            _thread.start_new_thread(bananagotchi.restart, (mode,))
