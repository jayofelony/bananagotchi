...
def load_config(args):
    ...
    import bananagotchi
    ref_defaults_file = os.path.join(os.path.dirname(bananagotchi.__file__), 'defaults.toml')
    ...
    for boot_conf in ['/boot/config.yml', '/boot/firmware/config.yml', '/boot/config.toml', '/boot/firmware/config.toml']:
        if os.path.exists(boot_conf):
            if os.path.exists(args.user_config):
                # if /etc/bananagotchi/config.toml already exists we just merge the new config
    ...
    # check for an entire bananagotchi folder on /boot/
    if os.path.isdir('/boot/firmware/bananagotchi'):
        print("installing /boot/firmware/bananagotchi to /etc/bananagotchi ...")
        shutil.rmtree('/etc/bananagotchi', ignore_errors=True)
        shutil.move('/boot/firmware/bananagotchi', '/etc/')
...
def extract_from_pcap(path, fields):
    ...
    for field in fields:
        ...
        if field == WifiInfo.BSSID:
            ...
            from bananagotchi.mesh.wifi import freq_to_channel
...
class StatusFile(object):
    ...
    def update(self, data=None):
        from bananagotchi.fs import ensure_write
