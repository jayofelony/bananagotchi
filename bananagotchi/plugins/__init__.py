...
def toggle_plugin(name, enable=True):
    ...
    """
    import bananagotchi
    from bananagotchi.ui import view
    from bananagotchi.utils import save_config

    global loaded, database

    if bananagotchi.config:
        if name not in bananagotchi.config['main']['plugins']:
            bananagotchi.config['main']['plugins'][name] = dict()
        bananagotchi.config['main']['plugins'][name]['enabled'] = enable
        save_config(bananagotchi.config, '/etc/bananagotchi/config.toml')
    ...
    if enable and name in database and name not in loaded:
        ...
        if name in loaded and bananagotchi.config and name in bananagotchi.config['main']['plugins']:
            loaded[name].options = bananagotchi.config['main']['plugins'][name]
        one(name, 'loaded')
        if bananagotchi.config:
            one(name, 'config_changed', bananagotchi.config)
