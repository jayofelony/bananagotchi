...
from bananagotchi.utils import WifiInfo, FieldNotFoundError, extract_from_pcap, StatusFile, remove_whitelisted
from threading import Lock
from bananagotchi import plugins
from bananagotchi._version import __version__ as __bananagotchi_version__
...
def _transform_wigle_entry(gps_data, pcap_data, plugin_version):
    ...
    dummy = StringIO()
    # write kismet header
    dummy.write(f"WigleWifi-1.6,appRelease={plugin_version},model=bananagotchi,release={__bananagotchi_version__},"
                f"device={bananagotchi.name()},display=kismet,board=RaspberryPi,brand=bananagotchi,star=Sol,body=3,subBody=0\n")
    dummy.write(
...
def _send_to_wigle(lines, api_key, donate=True, timeout=30):
    ...
    payload = {'file': (bananagotchi.name() + ".csv", dummy, 'multipart/form-data', {'Expires': '0'})}
    try:
