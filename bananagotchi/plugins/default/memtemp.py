...
###############################################################
from bananagotchi.ui.components import LabeledValue, Text
...
class MemTemp(plugins.Plugin):
    ...
    def mem_usage(self):
        return f"{int(bananagotchi.mem_usage() * 100)}%"

    def cpu_load(self):
        return f"{int(bananagotchi.cpu_load() * 100)}%"
    ...
    def cpu_temp(self):
        if self.options['scale'] == "fahrenheit":
            temp = (bananagotchi.temperature() * 9 / 5) + 32
            symbol = "f"
        elif self.options['scale'] == "kelvin":
            temp = bananagotchi.temperature() + 273.15
            symbol = "k"
        else:
            # default to celsius
            temp = bananagotchi.temperature()
            symbol = "c"
