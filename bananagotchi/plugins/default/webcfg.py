...
import _thread
import bananagotchi
from bananagotchi import restart, plugins
from bananagotchi.utils import save_config, merge_config
...
class WebConfig(plugins.Plugin):
    ...
    def on_webhook(self, path, request):
        ...
        if request.method == "GET":
            ...
            if path == "save-config":
                try:
                    save_config(request.get_json(), '/etc/bananagotchi/config.toml')  # test
                ...
                try:
                    ...
                    bananagotchi.config = merge_config(request.get_json(), bananagotchi.config)
                    logging.debug("bananagotchi CONFIG:\n%s" % repr(bananagotchi.config))
                    if self._agent:
