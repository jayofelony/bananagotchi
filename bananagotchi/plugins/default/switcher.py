...
from bananagotchi import reboot
...
def run_task(name, options):
    ...
    with open('/etc/systemd/system/%s' % task_service_name, 'wt') as task_service:
        task_service.write("""
        [Unit]
        Description=Executes the tasks of the bananagotchi switcher plugin
        After=bananagotchi.service bettercap.service
    ...
    if 'reboot' in options and options['reboot']:
        ...
        # add condition
        systemd_dropin("bananagotchi.service", """
        [Unit]
